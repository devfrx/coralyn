...
| **Prenotazione** | `Booking` | Impegno di un Ombrellone per una Fascia in un periodo definito. | Tipi (`type`): giornaliera/periodica/abbonamento. **Giornaliera implementata** (slice A1); periodiche/abbonamenti e il Pacchetto arrivano con A3/A4. Stato `confirmed`/`cancelled`. |
