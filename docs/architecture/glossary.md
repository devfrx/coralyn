...
| **Prenotazione** | `Booking` | Impegno di un Ombrellone per una Fascia in un periodo definito. | Tipi (`type`): giornaliera/periodica/abbonamento. **Giornaliera implementata** (slice A1); periodiche/abbonamenti arrivano con A4. `Booking.packageId` nullable, FK; **selettore Pacchetto implementato** (A3.2). Stato `confirmed`/`cancelled`. |
