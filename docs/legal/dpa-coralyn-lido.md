...
| Ambito | Misura |
|---|---|
| **Separazione tra clienti** **[V]** | Row-Level Security PostgreSQL con policy di isolamento per tenant in `ENABLE` + `FORCE` su **22 tabelle di dominio**; il ruolo applicativo è non-superuser e non può aggirare le policy in lettura o scrittura ordinaria ([ADR-0010](../architecture/decisions/0010-isolamento-multi-tenant.md)). **Restano fuori RLS 7 tabelle, nominatamente:** `User`, `Establishment`, `CredentialSetupToken`, `PlatformAuditLog` (identità e pre-tenant), `CustomerEnrollmentToken`, `CustomerSession` (credenziali del canale cliente) e `StaffPermissionOverride` (permessi assegnati al singolo operatore). Per le 22 tabelle di dominio, un difetto del solo codice applicativo non basta a esporre i dati di un altro Lido; **per le altre 7, sì**, perché il loro isolamento è garantito dal filtro applicativo. Per `StaffPermissionOverride` il database aggiunge una garanzia **in scrittura**: un vincolo di integrità referenziale composito impedisce di creare una riga che dichiari un Lido diverso da quello del proprio operatore ([ADR-0063](../architecture/decisions/0063-permessi-staff-configurabili-per-operatore.md)). ⚠️ Quel vincolo **non protegge la lettura**: una query applicativa scritta senza filtro sul Lido potrebbe comunque leggere righe altrui, esattamente come per le altre sei. ⚖️-09 |
...
| **Controllo degli accessi** **[V]** | Autenticazione obbligatoria su ogni endpoint applicativo, con guardia globale. Sono pubbliche per necessità funzionale le sole rotte di autenticazione, il controllo di stato del servizio e l'endpoint che espone i dati del titolare per l'informativa art. 13 (dati destinati per definizione alla pubblicazione verso l'interessato). Ogni endpoint dichiara il permesso che richiede e **in assenza di dichiarazione l'accesso è negato**: una rotta nuova nasce protetta, e una verifica automatica in integrazione continua rifiuta il codice che non dichiara. Le azioni amministrative restano riservate al ruolo amministratore ([ADR-0057](../architecture/decisions/0057-autorizzazione-fail-closed-permessi.md)). |
...
| **Limite di frequenza** **[V]** | Attivo per indirizzo IP sulle rotte di autenticazione di entrambi i canali — cliente 10 richieste/60s, operatori 20/60s, con contatore separato per rotta — più il blocco a soglia sui tentativi di PIN errato. Un tentativo fallito restituisce sempre lo stesso esito generico e, sul login operatori, con lo stesso costo di calcolo anche per indirizzi inesistenti: il tempo di risposta non rivela quali utenze siano registrate ([D-027](../architecture/deferred.md), [D-029](../architecture/deferred.md)). |
| **Limiti dichiarati** | Per trasparenza verso il Titolare, che su queste misure fonda la propria valutazione ex artt. 28.1 e 32: (a) **non esiste un registro delle azioni compiute dallo staff del Lido** sui dati dei propri bagnanti ([D-047](../architecture/deferred.md)) — il Titolare non dispone quindi di una traccia applicativa di chi ha letto o modificato cosa; (b) **non esiste revoca di un token di accesso già emesso**: disabilitare un operatore lo lascia operativo fino alla scadenza della sessione, al massimo 8 ore ([D-026](../architecture/deferred.md)); (c) i **registri tecnici** conservano l'indirizzo email del destinatario in caso di errore di invio, con retention da definire. |
