...
| Voce | Contenuto |
|---|---|
| **Finalità** | Creare e gestire le utenze di accesso al gestionale e alla console di piattaforma; autenticazione; controllo degli accessi **per ruolo e, per il personale operativo dei lidi, per permessi assegnati al singolo operatore dall'amministratore del lido** ([ADR-0063](../architecture/decisions/0063-permessi-staff-configurabili-per-operatore.md)). |
...
| **Categorie di dati** | Email; hash della password (argon2id, mai la password); ruolo; stato di abilitazione; riferimento al lido (assente per il superuser di piattaforma); **per il personale operativo, i permessi concessi o revocati dall'amministratore del lido**. **Nessun nome, cognome, telefono o dato anagrafico.** |
...
> **Nota di minimizzazione (verificata sul modello dati, `schema.prisma:146-159`):** l'entità `User`
...
|---|---|
| **Isolamento multi-tenant** **[V]** | Row-Level Security PostgreSQL con policy `tenant_isolation` in `ENABLE` + `FORCE` su **tutte e 22 le tabelle di dominio tenant-scoped**. Il ruolo applicativo è **non-superuser e `NOBYPASSRLS`**: non può aggirare le policy in lettura o scrittura ordinaria ([ADR-0010](../architecture/decisions/0010-isolamento-multi-tenant.md)). **Restano deliberatamente fuori RLS 7 tabelle**: identità e pre-tenant (`User`, `Establishment`, `CredentialSetupToken`, `PlatformAuditLog`), credenziali del canale cliente (`CustomerEnrollmentToken`, `CustomerSession`) e i permessi assegnati al singolo operatore (`StaffPermissionOverride`); per queste l'isolamento è **applicativo**, non di database. Per `StaffPermissionOverride` il database aggiunge una garanzia **in scrittura** — un vincolo di integrità referenziale composito impedisce di creare una riga che dichiari un lido diverso da quello del proprio operatore ([ADR-0063](../architecture/decisions/0063-permessi-staff-configurabili-per-operatore.md)) — ma **non in lettura**, che resta filtrata dall'applicazione come per le altre sei. ⚠️ Il ruolo applicativo è anche **proprietario dello schema**, quindi in linea teorica potrebbe alterare le policy con SQL grezzo: è il compromesso necessario a far girare le migration. ⚖️-09 |
| **Password** **[V]** | Hashing **argon2id**; mai memorizzate, trasmesse o registrate in chiaro. Consegna via link monouso a scadenza. |
| **Autenticazione** **[V]** | Guardia globale su ogni endpoint applicativo. Il marcatore `@Public()` **scavalca la guardia staff, non l'autenticazione in sé**: dei 13 handler che lo usano, 5 sono endpoint di dominio del canale cliente, autenticati dal `CustomerJwtGuard`. Restano **effettivamente non autenticati** solo: le rotte di autenticazione, il controllo di stato del servizio e l'endpoint che espone i dati del titolare per l'informativa art. 13 (dati destinati per definizione alla pubblicazione verso l'interessato). **Ogni endpoint applicativo dichiara esplicitamente il permesso che richiede, e in assenza di dichiarazione l'accesso è negato** — una rotta nuova nasce quindi protetta, non aperta; una verifica automatica in integrazione continua rifiuta il codice che non dichiara ([ADR-0057](../architecture/decisions/0057-autorizzazione-fail-closed-permessi.md)). |
...
| **Limite di frequenza** **[V]** | Attivo **per indirizzo IP** sugli endpoint di **autenticazione** di entrambi i canali: canale cliente 10 richieste/60s predefinite, rotte pubbliche di autenticazione degli operatori 20/60s predefinite, con contatore separato per ogni rotta. In più, blocco a soglia sui tentativi di PIN errato (5 predefiniti). Il numero di reverse proxy davanti all'applicazione è dichiarato in configurazione, condizione senza la quale il limite ricadrebbe su un unico contatore comune a tutti gli utenti invece che per indirizzo. ⚠️ Gli endpoint **di dominio** del canale cliente restano fuori dal limite di frequenza: sono protetti dal token di sessione ([D-027](../architecture/deferred.md)). |
| **Anti-enumerazione delle utenze** **[V]** | Un tentativo di accesso fallito restituisce sempre lo stesso esito generico, su entrambi i canali. Sul login degli operatori la verifica crittografica della password viene eseguita **anche quando l'indirizzo email non esiste**, così che il tempo di risposta non riveli quali indirizzi siano registrati ([D-029](../architecture/deferred.md)). |
