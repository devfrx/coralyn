...
> Documento di consegna per la **prossima sessione**. Due slice sono in sospeso su branch separati:
> **(1) "Archiviazione pacchetti"** — COMPLETO, rivisto (0 Critical/Important), live-verificato, **NON ancora mergiato**
> (branch `catalog-archiviazione-pacchetti`, per scelta dell'utente). **(2) "Pricing — Abbonamento partizione tipo"** —
> **spec di design approvata** (decisioni risolte con l'utente), **da pianificare ed eseguire**.
>
...
- **`main` = `origin/main`** (al momento della scrittura `6482e13`).
- **Branch `catalog-archiviazione-pacchetti`** (LOCALE, da `main`): 4 commit — `42122c5` (piano) · `9bc1594` (backend) ·
  `a1fe514` (FE) · `c0a5ba0` (fix nome pacchetti archiviati nello storico). **Done + reviewed + live-verified, non
  mergiato.** Questo branch porta anche, come commit doc, la spec del pricing e QUESTO handoff (vedi sotto).
- **Migrazione dell'archiviazione già applicata** a `coralyn_dev` e `coralyn_test` (`20260702204532_add_package_archived_at`,
  `Package.archivedAt` nullable). `prisma migrate status` pulito su entrambi.
- **Prossimo ADR libero:** **0035** lo consuma il pricing (§3). Dopo il pricing: **0036**. **Prossimo D libero:** **D-035**.

## 1. Stato dei due slice in sospeso
- **"Archiviazione pacchetti"** (branch, non mergiato): soft-delete reversibile (`Package.archivedAt`) + hard-delete
  esplicito (200 solo se archiviato+0rif, altrimenti 409); `listPackages` default solo attivi + `?includeArchived=true`;
  `POST /packages/:id/archive|restore`; FE editor con "Archivia" (no conferma) + sezione "Archiviati (N)" a scomparsa
  (Ripristina + Elimina definitivamente con ConfirmDialog); `useEntityLabels` usa `useAllPackages` così lo storico
  prenotazioni risolve i nomi archiviati. **Test live (branch HEAD `c0a5ba0`): api unit 91 · e2e 129 · web-staff 148 ·
  ui-kit 55 · typecheck pulito.** Review finale opus: ready = with fixes → **il fix è stato applicato** (regressione
  nomi archiviati) → ora pulito.
- **"Pricing — Abbonamento partizione tipo"**: spec **approvata**
  [`docs/specs/2026-07-02-pricing-abbonamento-partizione-tipo-design.md`](../specs/2026-07-02-pricing-abbonamento-partizione-tipo-design.md).
  Decisioni risolte (spec §7). Resta: **piano TDD + esecuzione** (subagent-driven, da nuovo branch da `main`).

## 2. LA SEQUENZA (risposta a "prossimo passo tra slice e D-0xx")
**Gli slice vengono PRIMA dei D-0xx.** Ordine consigliato:

0. **(Housekeeping) FF-merge di `catalog-archiviazione-pacchetti` → `main`** (è done+verificato). Porta su `main` anche la
   spec del pricing e questo handoff. Poi elimina il branch. *(Se l'utente vuole ancora rivederlo prima, questo passo
   attende la sua conferma — è la sua decisione.)*
1. **Slice "Pricing — Abbonamento partizione tipo"** — **spec pronta**, `pianifica + esegui` (§3). È un **bug di prezzo dal
   vivo** (abbonamento prezzato €28 invece di €800), piccolo e ad alto valore. **Questo è il prossimo passo reale.**
2. **Slice C "Equipment personalizzato"** — editor "voce+quantità" sul JSONB `Package.equipment`. **Brainstorming+spec
   prima** (decisione con l'utente: free-form JSONB — rec: YAGNI — vs entità `EquipmentType` → nuovo ADR). Confina con
   `Package` (archiviazione) e con **D-012**.
...
vengono dopo gli slice; il primo naturale sarà **D-034** (forfait periodico), che è il gemello del lavoro sul pricing/tipo.
...
1. **Motore** `isApplicable` ([`pricing.engine.ts:32`](../../apps/api/src/catalog/pricing.engine.ts:32)): per `ctx.type ===
   'subscription'` applicabili solo le tariffe `r.type === 'subscription'`; daily/periodic invariati (wildcard applica).
   **Precedenza invariata** (la partizione basta; niente riordino — spec §3.2).
...
4. **Test (spec §5):** unit motore — sostituisci 1:1 il test "subscription→forfait" (usa una tariffa `subscription`, non
   wildcard); aggiungi: €800 batte €28 fascia-specifica; subscription+solo-catch-all→NO_RATE; daily/periodic invariati. e2e:
   subscription senza tariffa Abbonamento → 422 specifico; €800/€850 invariati.
5. **Impatto (verificato):** seed dev + helper e2e hanno già tariffe subscription → nessun cambio dati; **1 solo test unit**
   aggiornato 1:1; resto additivo. Baseline `main`: api unit 89 · e2e 126 (da non regredire; attesi +3/4 unit, +1/2 e2e).
...
- **Comandi test** (root, `corepack pnpm`): api unit `--filter @coralyn/api test`; api e2e `--filter @coralyn/api test:e2e`
  (auto-carica `.env.test` al ROOT del repo); web-staff `--filter web-staff test`; typecheck `--filter web-staff typecheck`.
  La suite `web-staff` globa gli spec `ui-kit` (145/148 INCLUDONO i 55 di ui-kit). Il "worker failed to exit gracefully" di
  Jest è rumore di teardown pre-esistente, non un fallimento.
- **`.env.test` è al ROOT del repo** (non in `apps/api/`). Per comandi prisma sotto `--filter`, passa `DATABASE_URL`
  ...
  materializzazione di credenziali).

## 6. Ancore di codice (file:riga, VERIFICATE 2026-07-02/03)
- **Motore:** [`apps/api/src/catalog/pricing.engine.ts`](../../apps/api/src/catalog/pricing.engine.ts) — `isApplicable`
  **`:32-44`** (il check tipo è `:33`), `specificity` `:47-56` (INVARIATO), `resolvePrice` `:80-93`. Spec
...
dev (password admin container `coralyn-admin-8473`). **NB:** se il branch `catalog-archiviazione-pacchetti` non è ancora su
`main`, il primo passo (§2.0) è mergiarlo.
...
>
> STATO: due slice in sospeso su branch separati. (1) **"Archiviazione pacchetti"** COMPLETO, rivisto (0
> Critical/Important), live-verificato (api unit 91 · e2e 129 · web-staff 148 · ui-kit 55 · typecheck pulito), **NON ancora
> mergiato** (branch `catalog-archiviazione-pacchetti`). (2) **"Pricing — Abbonamento partizione tipo"**: spec di design
> **approvata** e committata (decisioni risolte con me), **da pianificare ed eseguire**.
>
...
> gotcha §5, ancore §6), poi la spec `docs/specs/2026-07-02-pricing-abbonamento-partizione-tipo-design.md`, poi ADR-0032
> (precedenza, che questo slice raffina con ADR-0035) e ADR-0006 (dominio).
>
> TASK, in sequenza: (0) se `catalog-archiviazione-pacchetti` non è su `main`, FF-mergialo (done+verificato) e presenta;
> (1) PIANIFICA (piano TDD) ed ESEGUI lo slice "Pricing — Abbonamento partizione tipo" (partizione dura del tipo: un
> abbonamento è prezzato solo da tariffe `type='subscription'`; motore `isApplicable`, 422 specifico, ADR-0035, test;
> precedenza e FE invariati); (2) poi Slice C "Equipment personalizzato" (brainstorming+spec, decisione free-form JSONB vs
> entità `EquipmentType` con me). Tieni i D-0xx DOPO gli slice: **D-034** (forfait periodico) è il gemello del pricing;
> **D-012** confina con Equipment; poi D-018/D-015/D-033/D-030. Workflow ADR-0009 per OGNI slice: spec → risolvi decisioni
> con me → piano TDD → subagent-driven, un commit per layer, test-first, da un NUOVO branch da main. Non regredire i
