...
L'app staff di **Coralyn** (il mio gestionale per stabilimenti balneari) — Vue 3 + TypeScript + Vite.
...
L'ho costruita su `@coralyn/ui-kit` (design system, token), `@coralyn/contracts` (DTO condivisi col
...
## In dev NON ho mock nel browser
...
**MSW lo tengo solo nei test**: l'ho agganciato da `vitest.config.ts` → `src/test/setup.ts`, con
`onUnhandledRequest: 'error'` — così una chiamata non mockata fa fallire il test invece di uscire in rete.
I mock stanno in `src/mocks/server.ts`.

In `main.ts` de-registro all'avvio, **solo in dev**, un eventuale service worker MSW rimasto da versioni
