...
const props = defineProps<{ rental: RentalDTO | null }>();

const settle = useSettleRentalPayment();
const amount = ref(0);
...
const total = computed(() => props.rental?.totalPrice ?? 0);
...
watch(open, (isOpen) => {
  if (isOpen && props.rental) {
    amount.value = props.rental.totalPrice;
    method.value = props.rental.paymentMethod ?? 'cash';
    date.value = props.rental.collectionDate ?? todayRome();
    error.value = '';
  }
});
...
<template>
  <Modal v-model:open="open" title="Registra incasso" eyebrow="Incasso noleggio">
    <div v-if="rental" class="flex flex-col gap-[18px]">
